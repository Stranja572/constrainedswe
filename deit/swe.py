...
class SWE_PoolingRun(nn.Module):
    def __init__(self, d_in, num_ref_points, num_projections):
        ...
        '''
        super(SWE_PoolingRun, self).__init__()
        self.d_in = d_in
        ...
        self.lambdas = torch.nn.Parameter(torch.zeros(num_projections))
    ...
    def forward(self, X):
        ...
        lambda_weights = torch.softmax(-self.lambdas, dim=0).unsqueeze(0).unsqueeze(-1).repeat(B, 1, M) # 1 x num_slices x 1 --> B x num_slices x M
        ...
        weighted_embeddings_refdim = self.weight(embeddings).sum(-1) # summed over reference elements, output is B x num_slices #L
