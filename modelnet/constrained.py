...
class ConstrainedSWE(nn.Module):
    def __init__(self, d_in, num_ref_points, num_projections, tau_softsort=1):
        '''
    ...
    def forward(self, X):
        ...
        ref_expanded = self.reference.unsqueeze(0).repeat(B, 1, 1) # B x M x D
        cost = torch.cdist(X, ref_expanded, p=2) # B x N x N (when M = N)
        ...
        for l in range(self.num_projections):
            sliced_X = Xslices_sorted_interpolated if M != N else Xslices
